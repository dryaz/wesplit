  ...
  async (event) => {
    ...
    try {
      ...
      const imagePromptTemplate =
        config.getString("image_prompt") ||
        'Generate a square flat vector-style icon about "{description}". The icon should look visually appealing and recognizable even at 128x128 resolution. No borders, paddings, or text.';

      console.log(
        `Generating image for group: ${groupId} | ${newImageDescription} | ${imageModel}, ${imageQuality}, ${imageResolution}, ${imageStyle}`
      );
      ...
      const publicUrl = `https://firebasestorage.googleapis.com/v0/b/${STORAGE_BUCKET}/o/${encodeURIComponent(
        fileName
...
exports.generateGroupToken = onCall(async (request) => {
  ...
  if (!groupId || !publicToken) {
    throw new HttpsError("invalid-argument", 'The function must be called with both "groupId" and "publicToken" arguments.');
  }
...
exports.recalculateBalances = functions.firestore.document("groups/{groupId}/expenses/{expenseId}").onWrite(async (change, context) => {
  const groupId = context.params.groupId;
...
async function initializeVerifier() {
  if (!signedDataVerifier) {
    ...
    signedDataVerifier = new SignedDataVerifier(appleRootCAs, enableOnlineChecks, environment, bundleId, appAppleId);
...
async function initializeVerifierDev() {
  if (!signedDataVerifierDev) {
    ...
    // Initialize SignedDataVerifier with the required parameters
    signedDataVerifierDev = new SignedDataVerifier(appleRootCAs, enableOnlineChecks, environment, bundleId, appAppleId);
...
exports.handleAppleServerNotificationDev = onRequest(async (req, res) => {
  try {
    ...
    const userSnapshot = await admin.firestore().collection("users").where("trxId", "==", originalTransactionId).limit(1).get();
...
exports.updateUserSubscription = functions.firestore.document("users/{userId}").onCreate(async (snap, context) => {
...
exports.applyPromoCode = functions.https.onCall(async (data, context) => {
  ...
  if (!promoCode || typeof promoCode !== "string") {
    ...
    throw new functions.https.HttpsError("invalid-argument", "The promo code must be a non-empty string.");
  }
  ...
  try {
    ...
    if (!promoDoc.exists) {
      ...
      throw new functions.https.HttpsError("not-found", "Promo code does not exist.");
    }
    ...
    if (typeof promoDays !== "number" || promoDays <= 0) {
      ...
      throw new functions.https.HttpsError("invalid-argument", 'Promo code has invalid "days" value.');
    }
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (!userDoc.exists) {
        ...
        throw new functions.https.HttpsError("invalid-data", "Can't find user document with provided ID in auth header");
      }
      ...
      if (appliedPromos.includes(promoCode)) {
        ...
        throw new functions.https.HttpsError("already-exists", "Promo code has already been used by this user.");
      }
      ...
      if (userData.expiresAt && userData.expiresAt.toMillis() > now.toMillis()) {
        ...
        newExpiresAt = admin.firestore.Timestamp.fromMillis(userData.expiresAt.toMillis() + promoDays * 24 * 60 * 60 * 1000);
        console.log(`Extending subscription for user ${uid} by ${promoDays} days.`);
      } else {
        // Set expiresAt to now + promoDays
        newExpiresAt = admin.firestore.Timestamp.fromMillis(now.toMillis() + promoDays * 24 * 60 * 60 * 1000);
    ...
    throw new functions.https.HttpsError("internal", "An internal error occurred while applying the promo code.");
  }
});
